          ...
          <div className="mt-24 mb-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {latestProducts.map((p) => {
                ...
                const pCoverImage = pImages && pImages.length > 0 ? pImages[0] : null;

                return (
                  <Link href={`/product/${p.id}`} key={p.id} className="bg-white rounded-[2.5rem] p-5 border border-slate-100 hover:shadow-2xl hover:border-slate-300 transition-all duration-500 group flex flex-col hover:-translate-y-2">
                    <div className="relative aspect-square bg-slate-50 rounded-3xl mb-6 flex items-center justify-center overflow-hidden">
                      {pCoverImage ? (
                        <img src={pCoverImage} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                      ) : (
